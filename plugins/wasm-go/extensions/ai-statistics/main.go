...
const (
	...
	LLMStreamDurationCount = "llm_stream_duration_count"package main
	...
	func writeMetric(ctx wrapper.HttpContext, config AIStatisticsConfig) {
		...
		log.Debugf("sourceIP resolved to: %s", sourceIP)
	...
	func classifyFailure(statusCode, codeDetails, transportFailure string) string {
		...
		switch {
		case code == 401:
			if strings.Contains(lowerDetails, "jwt") { return "auth_jwt_invalid" }
			if strings.Contains(lowerDetails, "ext_authz") || strings.Contains(lowerDetails, "auth") { return "auth_api_key_invalid" }
			return "auth_unauthorized"
		case code == 403:
			if strings.Contains(lowerDetails, "ext_authz") || strings.Contains(lowerDetails, "auth") { return "auth_forbidden" }
			if strings.Contains(lowerDetails, "ratelimit") || strings.Contains(lowerDetails, "rate_limit") { return "rate_limited" }
			if strings.Contains(lowerDetails, "rbac") { return "rbac_denied" }
			return "forbidden"
		case code == 429:
			return "rate_limited"
		}
		...
		// Routing failures
		if code == 404 {
			if strings.Contains(lowerDetails, "no_route") || strings.Contains(lowerDetails, "no cluster") { return "gateway_no_route" }
			return "not_found"
		}
	
		if code == 408 { return "request_timeout" }
	
		// Upstream errors (reached upstream but it returned 5xx or connection broke)
		if code >= 500 && code < 600 {
			if strings.Contains(lowerDetails, "via_upstream") { return "upstream_service_error" }
			if strings.Contains(lowerDetails, "connect_timeout") || strings.Contains(lowerDetails, "connect timeout") { return "upstream_connect_timeout" }
			if strings.Contains(lowerDetails, "connection_failure") || strings.Contains(lowerDetails, "connection failure") { return "upstream_connection_failure" }
			if strings.Contains(lowerDetails, "upstream_reset") || strings.Contains(lowerDetails, "upstream reset") { return "upstream_reset" }
			if strings.Contains(lowerDetails, "remote_disconnect") || strings.Contains(lowerDetails, "remote disconnect") { return "upstream_remote_disconnect" }
			if strings.Contains(lowerDetails, "no_route") || strings.Contains(lowerDetails, "no cluster") { return "gateway_no_route" }
			if strings.Contains(lowerDetails, "overload") { return "gateway_overload" }
			if strings.Contains(lowerDetails, "local_reply") || strings.Contains(lowerDetails, "local reply") { return "gateway_local_error" }
	...
	func summarizeMessages(value interface{}, maxBytes int) interface{} {
		...
		for i := len(arr) - 2; i < len(arr); i++ {
			if i >= 0 { buf.WriteString(","); buf.WriteString(arr[i].Raw) }
		}
		buf.WriteString("]")
		return buf.String()
	}
